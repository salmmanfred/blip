    ...
    for line in text.iter(){
        ...
        match line[0]{
            ...
            "IF" => {
                ...
                parsed.push(Command::If([curlin as i64,end as i64],[line[1].to_string(),line[2].to_string(),line[3].to_string()]))
            }
            "FN" => {
                let mut end = 0;
                for lines in curlin..text.len(){
            ...
            "MKV:I" =>{
                parsed.push(Command::MkvI([line[1].to_string(),line[2].to_string()]))
            }
            "GET" =>{
                match small::get_var_name(line[1].to_string()){
                    Some(_a) => {}
                    None => {panic!("Must be a variable, Line: {}",curlin)}
                };
            ...
            "RUN" =>{
                ...
                match pos{
                    ...
                    _=>{
                        panic!("This should never happen, if it has happen make an issue asap code 1")
                    }
                }
                line.remove(0);
                line.remove(0);
                let line: Vec<String> = line.iter().map(|x|x.to_string()).collect();
                parsed.push(Command::Run(line[1].to_string(),poss,line,args))
            ...
            "LOOP" =>{
                ...
                match pos{
                    ...
                    _=>{
                        panic!("This should never happen, if it has happen make an issue asap code 2")
                    }
                }
                ...
                let line: Vec<String> = line.iter().map(|x|x.to_string()).collect();
                parsed.push(Command::Loop(line[1].to_string(),poss,line,args,amount))
            }
            "DEL" =>{
                parsed.push(Command::Delete(line[1].to_string()))
            }
            "STOP" | "IF_STOP" | "FN_STOP" => {
                parsed.push(Command::Misc(Misc::Stop))
            }
            a =>{
                if a != ""{
